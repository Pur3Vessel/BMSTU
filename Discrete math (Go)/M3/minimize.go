package main
// Короче, очень важно нормальное проектирование проги, чтоб потом не сосать бибу, как я
// А я обрек себя на кучу костылей
import (
	"fmt"
	"sort"
)
...
func main() {
	...
	// Время позволяет сделать два канона)))
	...
	ratatatata(exitMin, crossoverMin, 0)
}
/*
США. В полном автобусе едут афроамериканцы и белые, между ними начинается потасовка.
Водитель останавливает автобус, заставляет всех выйти и построиться перед автобусом.
Спрашивает в чем дело.
Афроамериканцы:
— Да вот полная дискриминация, нас афроамериканцев постоянно унижают, говорят, мы белые будем сидеть спереди, а вы назад садитесь.
Водитель:
